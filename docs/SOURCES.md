...
**Export Portable Copy** builds a clean transferable ZIP containing the launchers, application source, documentation, optional downloaded runtime, and a `DoPie.dopie-profile`. The receiving copy imports it on first launch, asks for the transfer password once when required, creates a new folder-local vault key, and records the profile digest in `data/provisioning.json`. A changed profile is treated as new provisioning.
